...
class GetAllOrdersChart extends GetxController {
  ...
  Future<void> fetchMonthlyOrders() async {
    final CollectionReference ordersCollection = _firestore.collection(
        'orders');
    ...
    for (QueryDocumentSnapshot userSnapshot in allUserSnapshot.docs) {
      final QuerySnapshot userOrderSnapshot = await ordersCollection.doc(
          userSnapshot.id).collection('confirmOrders')
    .where('createdAt', isGreaterThanOrEqualTo: dateMonthAgo).get();
    ...
    final List<ChartData> monthlyData = monthlyCount.entries.map((entry)=> ChartData(entry.key, entry.value.toDouble())).toList();
